describe("ArrayWithExactContents", function() {
  it("matches an array with the same items in a different order", function() {
    ...
    expect(matcher.asymmetricMatch([2, 'a', /a/], [], jasmineUnderTest.matchersUtil)).toBe(true);
  });
  ...
  it("does not match when an item is missing", function() {
    ...
    expect(matcher.asymmetricMatch(['a', 2], [], jasmineUnderTest.matchersUtil)).toBe(false);
    expect(matcher.asymmetricMatch(['a', 2, undefined], [], jasmineUnderTest.matchersUtil)).toBe(false);
  });
  ...
  it("uses custom equality testers", function() {
    ...
    expect(matcher.asymmetricMatch(["fooBar"], [tester], jasmineUnderTest.matchersUtil)).toBe(true);
  });
});
