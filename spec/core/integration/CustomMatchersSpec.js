describe("Custom Matchers (Integration)", function() {
  ...
  describe("New style", function() {
    it("allows adding more matchers local to a spec", function(done) {
      env.it('spec defining a custom matcher', function() {
        env.addMatcher('matcherForSpec', function(matchersUtil, actual, expected) {
          return { pass: false, message: "matcherForSpec: actual: " + actual + "; expected: " + expected };
        });
    ...
    it("passes the spec if the custom matcher passes", function(done) {
      env.it("spec using custom matcher", function() {
        env.addMatcher('toBeReal', function(matchersUtil) {
          return { pass: true };
        });
    ...
    it("uses the negative compare function for a negative comparison, if provided", function(done) {
      env.it("spec with custom negative comparison matcher", function() {
        env.addMatcher('toBeReal', {
          compare: function(matchersUtil) { return { pass: true }; },
          negativeCompare: function(matchersUtil) { return { pass: true }; }
        });
    ...
    it("passes the expected and actual arguments to the comparison function", function(done) {
      ...
      env.it('spec with an expectation', function () {
        env.addMatcher('toBeReal', argumentSpy);
      ...
      var specExpectations = function() {
        expect(argumentSpy).toHaveBeenCalledWith(true, env.anything());
        expect(argumentSpy).toHaveBeenCalledWith(true, env.anything(), "arg");
        expect(argumentSpy).toHaveBeenCalledWith(true, env.anything(), "arg1", "arg2");
      };
