describe("matchersUtil", function() {
  describe("equals", function() {
    it("passes for literals that are triple-equal", function() {
      expect(jasmineUnderTest.matchersUtilFactory([]).equals(null, null)).toBe(true);
      expect(jasmineUnderTest.matchersUtilFactory([]).equals(void 0, void 0)).toBe(true);
    });

    it("fails for things that are not equivalent", function() {
      expect(jasmineUnderTest.matchersUtilFactory([]).equals({a: "foo"}, 1)).toBe(false);
    });

    it("passes for Strings that are equivalent", function() {
      expect(jasmineUnderTest.matchersUtilFactory([]).equals("foo", "foo")).toBe(true);
    });

    it("fails for Strings that are not equivalent", function() {
      expect(jasmineUnderTest.matchersUtilFactory([]).equals("foo", "bar")).toBe(false);
    });

    it("passes for Numbers that are equivalent", function() {
      expect(jasmineUnderTest.matchersUtilFactory([]).equals(123, 123)).toBe(true);
    });

    it("fails for Numbers that are not equivalent", function() {
      expect(jasmineUnderTest.matchersUtilFactory([]).equals(123, 456)).toBe(false);
    });

    it("passes for Dates that are equivalent", function() {
      expect(jasmineUnderTest.matchersUtilFactory([]).equals(new Date("Jan 1, 1970"), new Date("Jan 1, 1970"))).toBe(true);
    });

    it("fails for Dates that are not equivalent", function() {
      expect(jasmineUnderTest.matchersUtilFactory([]).equals(new Date("Jan 1, 1970"), new Date("Feb 3, 1991"))).toBe(false);
    });

    it("passes for Booleans that are equivalent", function() {
      expect(jasmineUnderTest.matchersUtilFactory([]).equals(true, true)).toBe(true);
    });

    it("fails for Booleans that are not equivalent", function() {
      expect(jasmineUnderTest.matchersUtilFactory([]).equals(true, false)).toBe(false);
    });
    ...
    it("fails for RegExps that are not equivalent", function() {
      expect(jasmineUnderTest.matchersUtilFactory([]).equals(/foo/, /bar/)).toBe(false);
      expect(jasmineUnderTest.matchersUtilFactory([]).equals(new RegExp("foo", "i"), new RegExp("foo"))).toBe(false);
    });

    it("passes for Arrays that are equivalent", function() {
      expect(jasmineUnderTest.matchersUtilFactory([]).equals([1, 2], [1, 2])).toBe(true);
    });
    ...
    it("passes for Arrays that are equivalent, with elements added by changing length", function() {
      ...
      expect(jasmineUnderTest.matchersUtilFactory([]).equals(foo, [undefined])).toBe(true);
    });

    it("fails for Arrays that have different lengths", function() {
      expect(jasmineUnderTest.matchersUtilFactory([]).equals([1, 2], [1, 2, 3])).toBe(false);
    });

    it("fails for Arrays that have different elements", function() {
      expect(jasmineUnderTest.matchersUtilFactory([]).equals([1, 2, 3], [1, 5, 3])).toBe(false);
    });
    ...
    it("fails for Arrays whose contents are equivalent, but have differing properties", function() {
      ...
      expect(jasmineUnderTest.matchersUtilFactory([]).equals(one, two)).toBe(false);
    });
    ...
    it("passes for Arrays with equivalent contents and properties", function() {
      ...
      expect(jasmineUnderTest.matchersUtilFactory([]).equals(one, two)).toBe(true);
    });

    it("passes for Errors that are the same type and have the same message", function() {
      expect(jasmineUnderTest.matchersUtilFactory([]).equals(new Error("foo"), new Error("foo"))).toBe(true);
    });

    it("fails for Errors that are the same type and have different messages", function() {
      expect(jasmineUnderTest.matchersUtilFactory([]).equals(new Error("foo"), new Error("bar"))).toBe(false);
    });
    ...
    it("fails for objects with different constructors", function() {
      ...
      expect(jasmineUnderTest.matchersUtilFactory([]).equals(new One(), new Two())).toBe(false);
    });

    it("passes for Objects that are equivalent (simple case)", function() {
      expect(jasmineUnderTest.matchersUtilFactory([]).equals({a: "foo"}, {a: "foo"})).toBe(true);
    });

    it("fails for Objects that are not equivalent (simple case)", function() {
      expect(jasmineUnderTest.matchersUtilFactory([]).equals({a: "foo"}, {a: "bar"})).toBe(false);
    });

    it("passes for Objects that are equivalent (deep case)", function() {
      expect(jasmineUnderTest.matchersUtilFactory([]).equals({a: "foo", b: { c: "bar"}}, {a: "foo", b: { c: "bar"}})).toBe(true);
    });

    it("fails for Objects that are not equivalent (deep case)", function() {
      expect(jasmineUnderTest.matchersUtilFactory([]).equals({a: "foo", b: { c: "baz"}}, {a: "foo", b: { c: "bar"}})).toBe(false);
    });
    ...
    it("passes for Objects that are equivalent (with cycles)", function() {
      ...
      expect(jasmineUnderTest.matchersUtilFactory([]).equals(actual, expected)).toBe(true);
    });
    ...
    it("fails for Objects that are not equivalent (with cycles)", function() {
      ...
      expect(jasmineUnderTest.matchersUtilFactory([]).equals(actual, expected)).toBe(false);
    });
    ...
    it("fails when comparing an empty object to an empty array (issue #114)", function() {
      ...
      expect(jasmineUnderTest.matchersUtilFactory([]).equals(emptyObject, emptyArray)).toBe(false);
      expect(jasmineUnderTest.matchersUtilFactory([]).equals(emptyArray, emptyObject)).toBe(false);
    });
    ...
    it("passes for equivalent Promises (GitHub issue #1314)", function() {
      ...
      expect(jasmineUnderTest.matchersUtilFactory([]).equals(p1, p1)).toBe(true);
      expect(jasmineUnderTest.matchersUtilFactory([]).equals(p1, p2)).toBe(false);
    });
    ...
    describe("when running in a browser", function() {
      ...
      it("passes for equivalent objects from different frames", function() {
        ...
        expect(jasmineUnderTest.matchersUtilFactory([]).equals({}, iframe.contentWindow.testObject)).toBe(true);
      ...
      it("fails for DOM nodes with different attributes or child nodes", function() {
        ...
        expect(jasmineUnderTest.matchersUtilFactory([]).equals(a,b)).toBe(false);
    ...
    describe("when running in Node", function() {
      ...
      it("passes for equivalent objects from different vm contexts", function() {
        ...
        expect(jasmineUnderTest.matchersUtilFactory([]).equals(sandbox.obj, {a: 1, b: 2})).toBe(true);
      });
      ...
      it("passes for equivalent arrays from different vm contexts", function() {
        ...
        expect(jasmineUnderTest.matchersUtilFactory([]).equals(sandbox.arr, [1, 2])).toBe(true);
      });
    });
    ...
    it("passes when Any is used", function() {
      ...
      expect(jasmineUnderTest.matchersUtilFactory([]).equals(number, anyNumber)).toBe(true);
      expect(jasmineUnderTest.matchersUtilFactory([]).equals(anyNumber, number)).toBe(true);
    });
    ...
    it("fails when Any is compared to something unexpected", function() {
      ...
      expect(jasmineUnderTest.matchersUtilFactory([]).equals(number, anyString)).toBe(false);
      expect(jasmineUnderTest.matchersUtilFactory([]).equals(anyString, number)).toBe(false);
    });
    ...
    it("passes when ObjectContaining is used", function() {
      ...
      expect(jasmineUnderTest.matchersUtilFactory([]).equals(obj, containing)).toBe(true);
      expect(jasmineUnderTest.matchersUtilFactory([]).equals(containing, obj)).toBe(true);
    });

    it("passes when an asymmetric equality tester returns true", function() {
      var tester = { asymmetricMatch: function(other) { return true; } };

      expect(jasmineUnderTest.matchersUtilFactory([]).equals(false, tester)).toBe(true);
      expect(jasmineUnderTest.matchersUtilFactory([]).equals(tester, false)).toBe(true);
    });

    it("fails when an asymmetric equality tester returns false", function() {
      var tester = { asymmetricMatch: function(other) { return false; } };

      expect(jasmineUnderTest.matchersUtilFactory([]).equals(true, tester)).toBe(false);
      expect(jasmineUnderTest.matchersUtilFactory([]).equals(tester, true)).toBe(false);
    });
    ...
    it("passes for an asymmetric equality tester that returns true when a custom equality tester return false", function() {
      ...
      expect(jasmineUnderTest.matchersUtilFactory([]).equals(asymmetricTester, true, [symmetricTester])).toBe(true);
    ...
    it("passes when an Any is compared to an Any that checks for the same type", function() {
      ...
      expect(jasmineUnderTest.matchersUtilFactory([]).equals(any1, any2)).toBe(true);
    });
    ...
    it("passes for null prototype objects with same properties", function () {
      ...
      expect(jasmineUnderTest.matchersUtilFactory([]).equals(objA, objB)).toBe(true);
    });
    ...
    it("fails for null prototype objects with different properties", function () {
      ...
      expect(jasmineUnderTest.matchersUtilFactory([]).equals(objA, objB)).toBe(false);
    });

    it("passes when comparing two empty sets", function() {
      jasmine.getEnv().requireFunctioningSets();
      expect(jasmineUnderTest.matchersUtilFactory([]).equals(new Set(), new Set())).toBe(true);
    });
    ...
    it("passes when comparing identical sets", function() {
      ...
      setB.add(6);
      setB.add(5);
      expect(jasmineUnderTest.matchersUtilFactory([]).equals(setA, setB)).toBe(true);
    });
    ...
    it("fails for sets with different elements", function() {
      ...
      expect(jasmineUnderTest.matchersUtilFactory([]).equals(setA, setB)).toBe(false);
    });
    ...
    it("passes when comparing two empty maps", function() {
      jasmine.getEnv().requireFunctioningMaps();
      expect(jasmineUnderTest.matchersUtilFactory([]).equals(new Map(), new Map())).toBe(true);
    });
    ...
    it("passes when comparing identical maps", function() {
      ...
      var mapB = new Map();
      mapB.set(6, 5);
      expect(jasmineUnderTest.matchersUtilFactory([]).equals(mapA, mapB)).toBe(true);
    });
    ...
    it("fails for maps with different elements", function() {
      ...
      expect(jasmineUnderTest.matchersUtilFactory([]).equals(mapA, mapB)).toBe(false);
    });
  ...
  describe("contains", function() {
    it("passes when expected is a substring of actual", function() {
      expect(jasmineUnderTest.matchersUtilFactory([]).contains("ABC", "BC")).toBe(true);
    });

    it("fails when expected is a not substring of actual", function() {
      expect(jasmineUnderTest.matchersUtilFactory([]).contains("ABC", "X")).toBe(false);
    });

    it("passes when expected is an element in an actual array", function() {
      expect(jasmineUnderTest.matchersUtilFactory([]).contains(['foo', 'bar'], 'foo')).toBe(true);
    });

    it("fails when expected is not an element in an actual array", function() {
      expect(jasmineUnderTest.matchersUtilFactory([]).contains(['foo', 'bar'], 'baz')).toBe(false);
    });

    it("passes with mixed-element arrays", function() {
      expect(jasmineUnderTest.matchersUtilFactory([]).contains(["foo", {some: "bar"}], "foo")).toBe(true);
      expect(jasmineUnderTest.matchersUtilFactory([]).contains(["foo", {some: "bar"}], {some: "bar"})).toBe(true);
    });

    it("uses custom equality testers if passed in and actual is an Array", function() {
      var customTester = function(a, b) {return true;};

      expect(jasmineUnderTest.matchersUtilFactory([]).contains([1, 2], 2, [customTester])).toBe(true);
    });

    it("fails when actual is undefined", function() {
      expect(jasmineUnderTest.matchersUtilFactory([]).contains(undefined, 'A')).toBe(false);
    });

    it("fails when actual is null", function() {
      expect(jasmineUnderTest.matchersUtilFactory([]).contains(null, 'A')).toBe(false);
    });
    ...
    it("passes with array-like objects", function() {
      ...
      expect(jasmineUnderTest.matchersUtilFactory([]).contains(capturedArgs, 'bar')).toBe(true);
    });
  });
  ...
  describe("buildMessage", function() {
    ...
    it("builds an English sentence for a failure case", function() {
      var actual = "foo",
        name = "toBar",
        message = jasmineUnderTest.matchersUtilFactory([]).buildFailureMessage(name, false, actual);
    ...
    it("builds an English sentence for a 'not' failure case", function() {
      var actual = "foo",
        name = "toBar",
        isNot = true,
        message = message = jasmineUnderTest.matchersUtilFactory([]).buildFailureMessage(name, isNot, actual);
    ...
    it("builds an English sentence for an arbitrary array of expected arguments", function() {
      var actual = "foo",
        name = "toBar",
        message = jasmineUnderTest.matchersUtilFactory([]).buildFailureMessage(name, false, actual, "quux", "corge");
